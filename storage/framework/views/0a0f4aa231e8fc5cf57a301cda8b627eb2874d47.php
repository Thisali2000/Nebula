                                ...
                                <div class="table-responsive">
                                    <form id="degreeTimetableForm" method="POST" action="<?php echo e(route('timetable.store')); ?>">
                                        ...
                                        <div class="text-center mt-4">
                                            ...
                                            <button type="button" class="btn btn-info me-2" id="downloadDegreePDF">
                                                <i class="ti ti-download"></i> Download PDF
                                            </button>
                                            <button type="button" class="btn btn-success" id="downloadDegreeExcel">
                                ...
                                <div class="table-responsive">
                                    <form id="certificateTimetableForm" method="POST" action="<?php echo e(route('timetable.store')); ?>">
                                        ...
                                        <div class="text-center mt-4">
                                            ...
                                            <button type="button" class="btn btn-info me-2" id="downloadCertificatePDF">
                                                <i class="ti ti-download"></i> Download PDF
                                            </button>
                                            <button type="button" class="btn btn-success" id="downloadCertificateExcel">
                                                <i class="ti ti-file-spreadsheet"></i> Download Excel
                                            </button>
                                        </div>
                                    </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateDropdownWithData(select, items, valueKey, textKey, placeholder) {
        console.log('Populating dropdown with data:', { items, valueKey, textKey, placeholder });
        select.innerHTML = `<option selected disabled value="">Select ${placeholder}</option>`;
        items.forEach(item => {
    ...
    degreeLocation.addEventListener('change', function() {
        console.log('Location changed to:', degreeLocation.value);
    ...
    degreeCourse.addEventListener('change', function() {
        console.log('Course changed to:', degreeCourse.value);
        resetAndDisable(degreeIntake, 'Select Intake');
        resetAndDisable(degreeSemester, 'Select Semester');
        degreeIntake.value = '';
        degreeSemester.value = '';
    ...
    degreeIntake.addEventListener('change', function() {
        console.log('Intake changed to:', degreeIntake.value);
        resetAndDisable(degreeSemester, 'Select Semester');
        degreeSemester.value = '';
        clearDateValidation();
    ...
    degreeSemester.addEventListener('change', function() {
        if (degreeSemester.value) {
            ...
            console.log('Selected semester option:', selectedOption);
            console.log('Start date dataset:', selectedOption.dataset.startDate);
            console.log('End date dataset:', selectedOption.dataset.endDate);
            ...
            if (selectedOption.dataset.startDate && selectedOption.dataset.endDate) {
                ...
                setTimeout(() => {
                    ...
                    console.log('Set start date to:', degreeStartDate.value);
                    console.log('Set end date to:', degreeEndDate.value);
                ...
                console.error('No start/end dates found in semester option');
                showToast('Error', 'Semester dates not available. Please contact administrator.', 'bg-danger');
                clearDateValidation();
            }
        } else {
            degreeTimetableSection.style.display = 'none';
    ...
    function fetchSemesters(courseId, intakeId, targetSelect) {
        console.log('Fetching semesters for course:', courseId, 'intake:', intakeId);
        showSpinner(true);
        fetch(`/timetable/get-semesters?course_id=${encodeURIComponent(courseId)}&intake_id=${encodeURIComponent(intakeId)}`)
            ...
            .then(data => {
                ...
                if (data.semesters && data.semesters.length > 0) {
                    ...
                    console.log('Processed semester options:', semesterOptions);
    ...
    function fetchWeeks(startDate, endDate) {
        console.log('Fetching weeks for date range:', startDate, 'to', endDate);
        showSpinner(true);
    ...
    document.getElementById('degreeTimetableForm').addEventListener('submit', function(e) {
        ...
        .catch(error => {
            ...
            showToast('Error', error.message || 'Failed to save timetable.', 'bg-danger');
        });
    });
    ...
    document.getElementById('certificateTimetableForm').addEventListener('submit', function(e) {
        ...
        console.log('Certificate timetable data being sent:', timetableData);
        formData.append('timetable_data', JSON.stringify(timetableData));
    ...
    document.getElementById('downloadDegreeExcel').addEventListener('click', function() {
        downloadTimetableExcel('degree');
    });

    document.getElementById('downloadCertificateExcel').addEventListener('click', function() {
        downloadTimetableExcel('certificate');
    });
    ...
    function downloadTimetableExcel(courseType) {
        // Get current filter values
        let filters = {};
        ...
        if (missingFields.length > 0) {
            showToast('Error', 'Please fill all required fields before downloading Excel.', 'bg-danger');
            return;
        }
        ...
        if (courseType === 'degree') {
            ...
            // Add week start date for Excel header
            if (degreeWeek.value) {
                const selectedWeekOption = degreeWeek.options[degreeWeek.selectedIndex];
                filters.week_start_date = selectedWeekOption.dataset.startDate;
            }
        ...
        const downloadUrl = `/download-timetable-excel?${params.toString()}`;
        
        // Create temporary link and trigger download
        const link = document.createElement('a');
        link.href = downloadUrl;
        link.download = `${courseType}_timetable_week_${filters.week_number || '1'}.xlsx`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        
        showSpinner(false);
        showToast('Success', `${courseType.charAt(0).toUpperCase() + courseType.slice(1)} timetable Excel download started!`, 'bg-success');
    }
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\HP\Desktop\Nebula\resources\views/timetable.blade.php ENDPATH**/ ?>