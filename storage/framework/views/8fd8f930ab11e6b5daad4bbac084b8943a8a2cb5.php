...
function calculateFinalAmount() {
    ...
    if (registrationFeeDiscountSelect && registrationFeeDiscountSelect.value) {
        ...
        if (discountType === 'percentage') {
            // Apply percentage discount to registration fee only
            const registrationFee = parseFloat(window.currentStudentData?.registration_fee || 0);
            const registrationFeeDiscount = registrationFee * (discountValue / 100);
            finalAmount = finalAmount - registrationFeeDiscount;
        } else if (discountType === 'amount') {
            // Apply fixed amount discount to registration fee only
            const registrationFee = parseFloat(window.currentStudentData?.registration_fee || 0);
            const registrationFeeDiscount = Math.min(discountValue, registrationFee); // Don't discount more than the registration fee
            finalAmount = finalAmount - registrationFeeDiscount;
        }
    }
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/Nebula finish/Nebula/resources/views/payment.blade.php ENDPATH**/ ?>